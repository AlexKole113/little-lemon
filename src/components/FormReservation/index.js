...
import {formRequestSuccessState, formRequestLoadingState, formRequestErrorState, formCompleteState} from "../../helpers/formStateSetupers";
import FormSubmitSuccesMessage from "../FormSubmitSuccesMessage";

const FormReservation = ({availableValues, requestDetails, dispatch}) => {


    const [formState, setFormState]     = useState({ success: null, loading: null, error: null , sent: null });
    const [submitCheck, setSubmitCheck] = useState({ isDateSelected: null, isTimeSelected: null, isGuestsSelected: null })

    const updateAvailableTime = (payload) => {
        setFormState(() => formRequestSuccessState() );
        dispatch({ type: mainActions.responseAPI.setAvailableTime.type, payload });
    }
    const setErrorFormMode = () => {
        setFormState(() => formRequestErrorState() );
        dispatch({ type: mainActions.responseAPI.setAvailableTime.type, payload: [] });
    }
    const serFormSentMode = () => {
        setFormState(() => formCompleteState() );
    }
    ...
    return(
        <section className={`main-reservation-group`}>
            <div className="container">
                {
                    formState.sent ? <FormSubmitSuccesMessage /> : <div className="reservation-form-group">
                        <div className="reservation-form-group_notify">
                            { formState.error && <div className={`danger-notification`}>
                        ...
                        <form onSubmit={onSubmit} className={`reservation-form`}>
                            <div className="reservation-form_fields">
                                <InputFormDate date={requestDetails.date} dispatch={dispatch} formMessage={onSubmitFieldNotFilled(submitCheck.isDateSelected , 'Please fill this field')} />
                                <SelectFormTime availableTime={availableTime} selectedTime={requestDetails.time} dispatch={dispatch} formMessage={onSubmitFieldNotFilled(submitCheck.isTimeSelected , 'Please fill this field')} />
                                <InputFormGuests guests={requestDetails.guests} dispatch={dispatch} formMessage={onSubmitFieldNotFilled(submitCheck.isGuestsSelected , 'Please fill this field')} />
