import './index.scss';
import mainActions from "../../actions/mainActions";
import {addToCart, removeFromCartByID} from "../../helpers/cartHelpers";


const AddToCartLink = ({ dispatch, isInCart, id, cart }) => {

    const onAddToCart      = () => dispatch({type: mainActions.cart.updateCart.type , payload: addToCart(cart, { id, amount: 1 }) });
    const onRemoveFromCart = () => dispatch({type: mainActions.cart.updateCart.type , payload: removeFromCartByID(cart,id) });


    return (
        <div className={`add-remove-group`} >
            {
                isInCart() ?
                    (<a aria-label="On Click" className={'remove-from-cart'} href="#" onClick={(e)=>{ e.preventDefault(); onRemoveFromCart() }}>
                        <span className={`remove-from-cart_text`}>Remove from Cart</span>
                        <svg xmlns="http://www.w3.org/2000/svg" height="16" width="12" viewBox="0 0 384 512"><path d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"/></svg>
                    </a>) :
                    (<a className={`add-to-cart`} href={'#'} onClick={(e)=>{ e.preventDefault(); onAddToCart() }} >
                        <span className={`add-to-cart_text`}>Order a delivery</span>
                        <svg xmlns="http://www.w3.org/2000/svg" height="16" width="20" viewBox="0 0 640 512"><path d="M312 32c-13.3 0-24 10.7-24 24s10.7 24 24 24h25.7l34.6 64H222.9l-27.4-38C191 99.7 183.7 96 176 96H120c-13.3 0-24 10.7-24 24s10.7 24 24 24h43.7l22.1 30.7-26.6 53.1c-10-2.5-20.5-3.8-31.2-3.8C57.3 224 0 281.3 0 352s57.3 128 128 128c65.3 0 119.1-48.9 127-112h49c8.5 0 16.3-4.5 20.7-11.8l84.8-143.5 21.7 40.1C402.4 276.3 384 312 384 352c0 70.7 57.3 128 128 128s128-57.3 128-128s-57.3-128-128-128c-13.5 0-26.5 2.1-38.7 6L375.4 48.8C369.8 38.4 359 32 347.2 32H312zM458.6 303.7l32.3 59.7c6.3 11.7 20.9 16 32.5 9.7s16-20.9 9.7-32.5l-32.3-59.7c3.6-.6 7.4-.9 11.2-.9c39.8 0 72 32.2 72 72s-32.2 72-72 72s-72-32.2-72-72c0-18.6 7-35.5 18.6-48.3zM133.2 368h65c-7.3 32.1-36 56-70.2 56c-39.8 0-72-32.2-72-72s32.2-72 72-72c1.7 0 3.4 .1 5.1 .2l-24.2 48.5c-9 18.1 4.1 39.4 24.3 39.4zm33.7-48l50.7-101.3 72.9 101.2-.1 .1H166.8zm90.6-128H365.9L317 274.8 257.4 192z"/></svg>
                    </a>)
            }
        </div>
    );

}

export default AddToCartLink;
