...
const AddToCartLink = ({ setCart, isInCart, id }) => {
    ...
    const addToCart      = () => setCart( prvState => ([...prvState, id ]) );
    const removeFromCart = () => setCart( prvState => prvState.filter( v => v !== id ) );
    ...
    return (
        <div className={`add-remove-group`} >
            {
                isInCart() ?
                    (<a className={'remove-from-cart'} href="#" onClick={(e)=>{ e.preventDefault(); removeFromCart() }}>
                    ...
                    (<a className={`add-to-cart`} href={'#'} onClick={(e)=>{ e.preventDefault(); addToCart() }} >
